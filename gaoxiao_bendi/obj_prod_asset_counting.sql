...
BEGIN 
    ...
    SET @sqlstr = CONCAT(@sqlstr , "	SELECT    t2.`fs_fuacct`, IFNULL(SUM(t1.fd_marketvalue ),0) ,(t3.fd_Totalbal ) FROM  (tcusecacct t2  LEFT JOIN ", tablename," ");     
    SET @sqlstr = CONCAT(@sqlstr , "  t1     ON t1.Fs_secacct = t2.Fs_secacct  AND t1.Fi_holdamount >0 )  JOIN   tcasub2balance t3 ON t3.fs_fuacct = t2.fs_fuacct 
							WHERE  t2.fs_usagecode = '",contestcode ,"'  GROUP BY t2.fs_fuacct  WITH ROLLUP ");    						
    PREPARE stmt FROM @sqlstr;      EXECUTE stmt;    
    DEALLOCATE PREPARE stmt;      
  OPEN cur ;
  read_loop :
  LOOP
    FETCH cur INTO fuacct,maketvalue,bal;
  ...
  COMMIT ;
	...
	WHERE t1.fs_fuacct = t2.fs_fuacct  and t2.Fs_raceid =contestcode ;
	UPDATE tdailyassetsstatistics  t1, tstatistics t2 
	SET t1.Fd_totalbal = t2. fd_todaybal, 
		t1.Fd_maketvalue = t2.fd_todaymarketvalue #
	WHERE t1.fs_fuacct = t2.fs_fuacct AND TO_DAYS(t1.Ft_time) = TO_DAYS(NOW()) AND t2.Fs_raceid =contestcode ;
