...
/*Table structure for table `thistoryuser` */

DROP TABLE IF EXISTS `thistoryuser`;

CREATE TABLE `thistoryuser` (
  ...
  `Fs_type` varchar(2) DEFAULT NULL COMMENT '账号类型:1:学校账号 2：机构账号 3:超级管理员',
  ...
  PRIMARY KEY (`fs_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='用户（管理员）历史数据表';
